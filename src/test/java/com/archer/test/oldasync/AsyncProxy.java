...
public class AsyncProxy {
	...
	public Class<?> newAsyncClass() {
		...
		for(int i = 0; i < superMethods.length; i++) {
			...
			if("RuntimeVisibleAnnotations".equals(m.getAttributes()[0].getName())) {
				...
				if(ASYNC_TYPE.equals(((ConstantUtf8)supercp[aindex]).getValue())) {
					...
					ClassBytecode task = generateAsyncTask(curClsName, m.getName(), m.getDesc());
					try {
						Files.write(Paths.get("D:/" + task.getSimpleName() + ".class"), task.encodeClassBytes().readAll());
					} catch (IOException e) {
						e.printStackTrace();
					}
					task.loadSelfClass();
		...
		try {
			Files.write(Paths.get("D:/" + newClassByteCode.getSimpleName() + ".class"), newClassByteCode.encodeClassBytes().readAll());
		} catch (IOException e) {
			e.printStackTrace();
		}
...
//				byte[] codes = InstructionTable.decodeLoadIns(args[i],  i + 1);
		...
		for(int i = 0; i < args.length; i++) {
			String name = "p" + i;
			fields[i + 1] = new MemberInfo(true);
...
//		newcls.setClassEnd(end);
	...
	private MemberInfo generateMethod(Map<String, Integer> nameIndexMap, String curClassType, String name, String desc, String args[], 
		...
		CodeAttribute catt = new CodeAttribute(0, 0, null);
		mattr[0] = catt;
		catt.setName("Code");
...
//		catt.setLength(20 + catt.getCodeLength());
		...
		LineNumAttribute lna = new LineNumAttribute(0, 0, null);
		cattAttr[0] = lna;
...
//		catt.setLength(catt.getLength() + lna.getLength() + 2);
		...
		lineIndex += gap;
		LocalVarAttribute lva = new LocalVarAttribute(0, 0, null);
		cattAttr[1] = lva;
